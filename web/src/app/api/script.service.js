 ...
 */
export default angular.module('thingsscript.api.script', ['ngMaterial'])
...
function ScriptService($http, $q, $rootScope, $filter, types, utils, userService) {
    ...
    function renameScript(scriptId, newName) {
        var deferred = $q.defer();
        var url = types.baseApiUrl + '/script/' + scriptId +'/rename/';
        var renameRequest = {
            new_name: newName
        };
        $http.post(url, renameRequest).then(function success(response) {
            invalidateScriptsCache();
            deferred.resolve(response.data);
        }, function fail(response) {
            deferred.reject(response.data);
